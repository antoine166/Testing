...
- Domains: read (`list_domains`) and create (`create_domain`) via MCP; rename/recolor/delete stay app-only for now. Projects remain read-only (`list_projects`). Routines, checklists, knowledge library, and attachments are not exposed via MCP yet — manage those in the app
