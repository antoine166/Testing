...
// Vercel Cron calls this with GET, authenticating via the CRON_SECRET env var
// that Vercel auto-injects as `Authorization: Bearer <CRON_SECRET>`.
// See vercel.json for the schedule.
export async function GET(request: Request) {
  const expected = process.env.CRON_SECRET;
  if (!expected) {
    return NextResponse.json({ error: "Recurring tasks cron isn't configured" }, { status: 503 });
  }
...
// Plain token-secured endpoint for manual/MCP-triggered calls (curl/Bash),
// same pattern as /api/digest — tops up every active recurring task
// template's generated-but-not-yet-due occurrences back up to its horizon.
// Safe to call as often as you like: a template with no deficit generates
